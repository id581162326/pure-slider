...
export default class implements M.Interface {
  ...
  private updateCurrents: (xs: M.Currents) => void = (currents) => {
    this.validateCurrentsByLength(currents);

    const corrected = pipe(currents, this.validateCurrentsByLength, this.correctCurrents);
  ...
  private validateCurrentsByLength: (xs: M.Currents) => M.Currents = (currents) => pipe(currents, this.validateByLength);
